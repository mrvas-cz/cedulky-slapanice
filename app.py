...
# --- 3. BEZPEČNÁ PAMĚŤ (Zde je ta hlavní oprava) ---
...
with tab1:
    ...
    with col_search:
        ...
        # Hodnoty nahráváme přímo ze stavu
        new_name = st.text_input("Název odrůdy:", value=get_val("name"))
        st.session_state.data["name"] = new_name
        ...
        if new_name and os.path.exists(os.path.join(DB_DIR, folder_check)):
            ...
            if st.button("📂 Načíst existující data", type="primary"):
                ...
                st.session_state.data.update(d)
                if "cat" not in st.session_state.data or st.session_state.data["cat"] not in KATEGORIE:
                    st.session_state.data["cat"] = "Ostatní"
                st.session_state.data["img"] = img
        ...
        cat_idx = KATEGORIE.index(get_val("cat")) if get_val("cat") in KATEGORIE else KATEGORIE.index("Ostatní")
        new_cat = st.selectbox("Kategorie pro uložení:", KATEGORIE, index=cat_idx)
        st.session_state.data["cat"] = new_cat

        uploaded_file = st.file_uploader("📸 Nahrát staženou fotku:", type=["jpg", "png", "jpeg"], key=st.session_state.uploader_key)
        if uploaded_file:
            st.session_state.data["img"] = Image.open(uploaded_file).convert("RGB")

        if st.session_state.data["img"]:
            st.image(st.session_state.data["img"], width=180, caption="Aktuální fotka na cedulku")

    with col_data:
        st.header("2. Obsah Cedulky")

        new_ai = st.text_area("Vložit výsledek z AI (automaticky se rozdělí):", value=st.session_state.ai_text, height=120)
        if new_ai != st.session_state.ai_text:
            st.session_state.ai_text = new_ai
            clean_text = new_ai.replace("**", "").replace("*", "")
            for line in clean_text.split('\n'):
                if "Ř1:" in line: st.session_state.data["r1"] = line.split("Ř1:")[1].strip()[:65]
                elif "Ř2:" in line: st.session_state.data["r2"] = line.split("Ř2:")[1].strip()[:65]
                elif "Ř3:" in line: st.session_state.data["r3"] = line.split("Ř3:")[1].strip()[:65]
                elif "Ř4:" in line: st.session_state.data["r4"] = line.split("Ř4:")[1].strip()[:65]
            st.rerun()

        new_r1 = st.text_input("Řádek 1 (Stanoviště/Zálivka):", value=get_val("r1"), max_chars=65)
        st.session_state.data["r1"] = new_r1

        new_r2 = st.text_input("Řádek 2 (Spon/Výška):", value=get_val("r2"), max_chars=65)
        st.session_state.data["r2"] = new_r2

        new_r3 = st.text_input("Řádek 3 (Plod/Hmotnost):", value=get_val("r3"), max_chars=65)
        st.session_state.data["r3"] = new_r3

        new_r4 = st.text_input("Řádek 4 (Použití/Tip):", value=get_val("r4"), max_chars=65)
        st.session_state.data["r4"] = new_r4
        ...
        with col_btn1:
            if st.button("💾 ULOŽIT DO SKLADU", use_container_width=True, type="primary"):
                if get_val("name") and get_val("img"):
                    p = os.path.join(DB_DIR, clean_filename(get_val("name")))
                    if not os.path.exists(p): os.makedirs(p)
                    d_out = {
                        "name": get_val("name"), "cat": get_val("cat"),
                        "r1": get_val("r1"), "r2": get_val("r2"),
                        "r3": get_val("r3"), "r4": get_val("r4")
                    }
                    with open(os.path.join(p, "data.json"), "w", encoding="utf-8") as f:
                        json.dump(d_out, f, ensure_ascii=False)
                    st.session_state.data["img"].save(os.path.join(p, "photo.jpg"), "JPEG")
    ...
    if get_val("name") and get_val("img"):
        ...
        with st.spinner("Generuji arch..."):
            ...
            lines = [get_val("r1"), get_val("r2"), get_val("r3"), get_val("r4")]
            valid_lines = [r for r in lines if r.strip() and not r.endswith(": | ")]
            if not valid_lines: valid_lines = lines

            single_lbl = draw_label(get_val("name"), get_val("img"), valid_lines, f_b, f_r)
            ...
            c_dl.download_button("📥 STÁHNOUT PDF K TISKU", pdf_buf.getvalue(), f"{clean_filename(get_val('name'))}_A4.pdf", mime="application/pdf", type="primary")
